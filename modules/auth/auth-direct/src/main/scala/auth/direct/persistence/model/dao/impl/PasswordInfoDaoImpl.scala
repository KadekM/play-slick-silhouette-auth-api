...
import auth.direct.persistence.model.dao.PasswordInfoDao
...
class PasswordInfoDaoImpl(
  ...
  override def update(loginInfo: SilhouetteLoginInfo,
                      authInfo: SilhouettePasswordInfo): Future[SilhouettePasswordInfo] =
  ...
  override def save(loginInfo: SilhouetteLoginInfo,
                    authInfo: SilhouettePasswordInfo): Future[SilhouettePasswordInfo] = {
    val query = findDbLoginInfo(loginInfo).joinLeft(passwordInfosQuery).on(_.id === _.loginInfoId)
  ...
  override def add(loginInfo: SilhouetteLoginInfo,
                   authInfo: SilhouettePasswordInfo): Future[SilhouettePasswordInfo] =
    db.run(addAction(loginInfo, authInfo)).map(_ ⇒ authInfo)

  protected def passwordInfoQuery(loginInfo: SilhouetteLoginInfo) =
    for {
